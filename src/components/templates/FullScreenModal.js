import React from "react";

const FullScreenModal = ({ isOpen, onClose }) => {
    if (!isOpen) return null;

    return (
        <header class="flex flex-col justify-between fixed z-30 top-0 left-0 right-0 bg-opacity-80 transition-padding-shadow lg:hidden backdrop-filter py-8 bottom-0 bg-gray-900 backdrop-blur-md backdrop-saturate-150">
            <div class="flex justify-between px-8"></div>
            <nav class="flex flex-col text-gray-400 font-semibold gap-y-2">
                <a
                    class="uppercase tracking-wider py-4 px-8 inline-block hover:text-sky-500 transition-colors group"
                    href="/"
                >
                    <span class="transform transition-transform inline-block group-hover:translate-x-2">
                        Home
                    </span>
                </a>
                <a
                    class="uppercase tracking-wider py-4 px-8 inline-block hover:text-sky-500 transition-colors group"
                    href="/projects"
                >
                    <span class="transform transition-transform inline-block group-hover:translate-x-2">
                        Projects
                    </span>
                </a>
                <a
                    class="uppercase tracking-wider py-4 px-8 inline-block hover:text-sky-400 transition-colors group"
                    href="/contact"
                >
                    <span class="transform transition-transform inline-block group-hover:translate-x-2">
                        Contact
                    </span>
                </a>
                <a
                    class="uppercase tracking-wider py-4 px-8 inline-block hover:text-sky-400 transition-colors group"
                    href="/resume"
                >
                    <span class="transform transition-transform inline-block group-hover:translate-x-2">
                        Resume
                    </span>
                </a>
            </nav>
            <nav class="text-gray-400 px-8 flex gap-5 mb-4">
                <a
                    href="https://github.com/maifizumaretiya2119"
                    class="py-5 px-3 hover:text-gray-50 transition-colors"
                    aria-label="GitHub Profile"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                    >
                        <path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path>
                    </svg>
                </a>
                <a
                    href="https://twitter.com/abhishekpowade"
                    class="py-5 px-3 hover:text-sky-400 transition-colors"
                    aria-label="Twitter Profile"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                    >
                        <path d="M23 3a10.9 10.9 0 0 1-3.14 1.53 4.48 4.48 0 0 0-7.86 3v1A10.66 10.66 0 0 1 3 4s-4 9 5 13a11.64 11.64 0 0 1-7 2c9 5 20 0 20-11.5a4.5 4.5 0 0 0-.08-.83A7.72 7.72 0 0 0 23 3z"></path>
                    </svg>
                </a>
                <a
                    href="https://www.linkedin.com/in/abhishekpowade/"
                    class="py-5 px-3 hover:text-blue-500 transition-colors"
                    aria-label="LinkedIn Profile"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                    >
                        <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path>
                        <rect x="2" y="9" width="4" height="12"></rect>
                        <circle cx="4" cy="4" r="2"></circle>
                    </svg>
                </a>
                <a
                    href="mailto:abhishekpowade@gmail.com"
                    class="py-5 px-3 hover:text-gray-50 transition-colors"
                    aria-label="E-mail"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                    >
                        <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path>
                        <polyline points="22,6 12,13 2,6"></polyline>
                    </svg>
                </a>
            </nav>
        </header>
    );
};

export default FullScreenModal;
